...
1. **MCP server runtime: in-process.** The Linear MCP server runs inside the Next.js Node.js runtime via lazy instantiation in `src/lib/mcp/linear-server.ts`. Rationale: lowest-risk way to prove the wrapper contract. Re-host to a separate Vercel process or external host in a later phase if coupling becomes a problem. The wrapper API stays stable either way.
2. **Credential storage: Vercel env var.** `LINEAR_API_KEY` as a plain env var. Phase 1a is single-tenant (`DEV_TENANT_ID`) so per-tenant credential tables are premature. A `linear_credentials` table with RLS lands alongside real auth in Phase 1.5.
3. **Wrapper → MCP auth: none.** Drops out of in-process — shared memory, no auth surface. If we later move to B/C runtime options, auth comes back into scope.
4. **Linear MCP server implementation: official package, exact-pinned.** If no official Linear MCP server exists, pick the highest-quality community implementation. If nothing meets the quality bar, escalate — we do not hand-roll an MCP server inside Phase 1b. Exact-pinned in `package.json`, no caret.
